# Global Mangrove Mapping

----------------------------------------------------------------------------------------------------------
## INTRODUCTION

The Global Mangrove Mapping (GMM) provides an 
...
image-based tidal calibration approach. The GMM is freely accessible for 
non-profit use and runs on comprehensive and thoroughly commented code within 
GEE. 

The GMM is designed specifically to map multi-date mangrove distributions
...
local expertise typically held by coastal managers - the GMM works 
...
GMM is forked from [GEM](https://github.com/Blue-Ventures-Conservation/GEEMMM).  
A pilot study (i.e., [Yancho et al. 2020](https://www.mdpi.com/2072-4292/12/22/3758)) 
published in a special issue on [“Remote Sensing in Mangroves”](https://www.mdpi.com/journal/remotesensing/special_issues/remote_sensing_mangroves)  in the journal Remote Sensing demonstrates an application of the Google Earth Engine Mangrove Mapping Methology(GEM) for the entire coast of Myanmar (Burma) - a global mangrove loss hotspot.  The  published manuscript not only demonstrates one application for GEM, but 
also describes in detail the various parameters, options, outputs, and  user-interface features included in the tool. 

GMM has made some improvements based on [GEM](https://github.com/Blue-Ventures-Conservation/GEEMMM). These improvements mainly improve the adaptation range of the GEM model, as well as the biomass estimation of mangrove projects to meet the requirements of carbon credit accounting, verification and issuance by adding some auxiliary data.
...
Separate from the three modules of the GMM, there is also a ‘Functions’ 
...
which also provides further detail on how to best run the GMM for your 
...
allow the user to more comprehensively understand GMM functionality - 
this is not required to actually use the tool.

For new users to GEE, there are several sources available to learn how the 
service works. The GMM has been designed to minimize knowledge requirements
...
the GMM, or learn more about default datasets available to GMM users. 
...
It is likely that most casual users of the GMM will not encounter issues that 
...
errors which users may encounter while using the GMM. Please read the ‘KNOWN 
ISSUES’ section of this document before contacting the developers.

The development of the GMM was undertaken using the latest versions of both
...
functionality of the GMM on Linux systems or other web browsers is unknown at
...
Additionally, the GMM is designed to be universally applicable to any location, 
globally, where mangroves occur. However, it should be noted that the pilot study,
and subsequently the default parameters currently associated with the GMM, 
...
## OPERATION
Before the GMM can be used, the user must request developer access to GEE. 
...
days. Once your Google account has been granted access to GEE, you can use the GMM.
...
At this time, the GMM is not available for commercial use - interested parties 
should contact the authors to confirm this status. 

There are two methods to run the GMM:
...
The second option for using the GMM is to copy each of the three Module 
...
the script dependencies’ locations. If the user chooses to run the GMM in 
...
#### Example:
    ...
    var known_ext =  require('users/yanchojo/GMM_LS9/:modules/known_mangroves');
    var coastline =  require('users/yanchojo/GMM_LS9/:modules/coastline');

    //Once you have copied and saved your version of the GMM code
...
not done for all 13 function scripts the GMM will not work properly, likely 
...
After you have opened the links or saved the GMM locally, there are detailed 
...
within the GMM will operate. Throughout each module, there are comments at the 
end of each distinct section and step identifying the end of the user inputs/instruction.

The data inputs for the GMM have both internal and external source options, 
...
their choice, and subsequently point the GMM towards these data.  Whether using 
...
processing large areas. While developing the GMM it was determined that
...
stability and run-time challenge when using the GMM; however, it remains much 
faster than standalone workstations, and once one portion of the GMM initiates 
...
GMM. It has been documented that Firefox will have trouble loading calculated 
